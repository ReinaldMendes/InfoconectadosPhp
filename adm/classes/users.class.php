...
class Users {
    ...
    public function adicionar($email, $nome, $senha, $permissoes) {
        ...
        if (count($emailExistente) == 0) {
            try {
                ...
                $sql = $this->con->conectar()->prepare("INSERT INTO users(nome, email, senha, permissoes) VALUES(:nome, :email, :senha, :permissoes)");
    ...
    public function editar($nome, $email, $senha, $permissoes, $id) {
        ...
        if (count($emailExistente) > 0 && $emailExistente['id'] != $id) {
            ...
            try {
                $sql = $this->con->conectar()->prepare("UPDATE users SET nome = :nome, email = :email, senha = :senha, permissoes = :permissoes WHERE id = :id");
    ...
    public function getNomeUsuario() {
        $sql = $this->con->conectar()->prepare("SELECT nome FROM users WHERE id = :id");
